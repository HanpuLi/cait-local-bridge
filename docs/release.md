...
Release assets are append-once from the workflow's point of view: rerunning a tag workflow does not clobber an existing GitHub Release. The build step pins the validated `build`/ `twine` versions used by the current release path, and every artifact handoff is checked against `SHA256SUMS`. The MCP Registry publisher is version-pinned and its downloaded archive is checked against the upstream release SHA-256 before execution.
