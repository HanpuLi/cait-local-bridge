...
| OpenSSF criticality | 0.12854 measured with OpenSSF `criticality_score` v2.0.4 using GitHub signals only (`deps.dev` disabled) | >= 0.4 for a maintained repository | measured shortfall 0.27146; the full deps.dev-enriched default score is still unavailable because Google Cloud ADC is not configured | `metrics/oss-health.json`; official OpenSSF CLI |
