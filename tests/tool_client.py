"""Minimal MCP tool client for local testing (defaults: production bridge :8795; set CLB_BASE / CLB_PASSPHRASE): performs an OAuth flow in memory, then
exposes `call(name, **args)`. Tokens are never persisted by this helper. Usage:
...
def token() -> str:
    result = oauth_flow(BASE, os.environ["CLB_PASSPHRASE"], PUBLIC)
